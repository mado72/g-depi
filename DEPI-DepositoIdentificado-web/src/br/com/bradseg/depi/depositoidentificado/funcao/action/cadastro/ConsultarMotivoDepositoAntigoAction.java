...
@Controller
@Scope("request")
public class ConsultarMotivoDepositoAntigoAction extends ActionSupport implements SessionAware, ServletRequestAware {

	private static final Logger LOGGER = LoggerFactory.getLogger(ConsultarMotivoDepositoAntigoAction.class);
	...
	private MotivoDepositoFiltroForm filtro = new MotivoDepositoFiltroForm();
	...
	public MotivoDepositoFiltroForm getFiltro() {
		return filtro;
	}

	public String menu() {
		
		MotivoDepositoFiltroForm model = (MotivoDepositoFiltroForm) request.getAttribute(MotivoDepositoFiltroForm.NOME_FORM);
		
		if (model != null) {
			session.put(MotivoDepositoFiltroForm.NOME_FORM, model);
		}
		
		request.setAttribute("DepositoForm", DepositoCampo.valuesForCriteria());
		request.setAttribute("subtitulo", "Motivo Depósito - Cadastramento");
