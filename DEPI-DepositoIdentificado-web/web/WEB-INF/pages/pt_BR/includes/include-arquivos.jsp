...
//MÃ©todo responsÃ¡vel por formatar um CNPJ ou CPF.
...
//MÃ©todo responsÃ¡vel por mudar o parametro acao, action, target do form e submeter. 
...
//MÃ©todo responsÃ¡vel por mudar o parametro acao do form e submeter. 
			...
			if (el.type=="text" || el.type=="textarea" || el.type=="select" || el.type=="select-one" || el.type=="checkbox" || el.type=="radio") {
				...
				if (el.type=="text") {		   		
					if (el.onkeypress == undefined) {
						...
						//Entende o aspecto do controle e define aï¿½ï¿½es padrÃ£o para os eventos.
						...
						if (re1.test(new String(el.onkeypress))) {
							if (el.onblur == undefined) {
								el.onblur = function () {if(!checkDate(this) && this.value != '') {alert('Data invÃ¡lida: ' + this.value); this.value = '';}};
						 	}
							if (el.onkeyup == undefined) {
...
//MÃ©todo responsÃ¡vel desabilitar a tecla enter. 
...
//MÃ©todo responsÃ¡vel por redirecionar para uma determinada tela. 
...
function permitirApenasLetras(e) {
	var isIE = (window.event)? true : false;
	var keyNumber = (isIE) ? event.keyCode : ((window.event)? event : ((window.event)? event : ((window.event)? event : e))).which;
